...
**Кроки:** Надіслати запит без обов’язкового поля, з некоректним email або слабким паролем (менше 8 символів, не містить хоч одну uppercase, lowercase chars та цифри)
...
**Кроки:** Повторити реєстрацію з тим самим email

**Очікуваний результат:**

- повертається 409 без вказання інформації що така електронна адреса вже зареєстрована (з міркувань безпеки)
...
- час життя token відповідає конфігурації (наприклад 20 хв)
...
- повертається 403 permission denied
...
- запис події присутній в audit log
...
- повертається повідомлення про недостатність коштів;
...
- спрацьовує визначена політика: rejection
...
- повтор зафіксований в audit logs
